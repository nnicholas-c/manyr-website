...
export default function DocsPage() {
  return (
    <div className="min-h-screen pt-20 pb-16 relative">
      <BlurredEllipses
        ellipses={[
          { color: '#F5EFE0', size: 500, x: '0%', y: '20%', parallaxStrength: 0.05 },
          { color: '#EDE8F5', size: 400, x: '80%', y: '50%', parallaxStrength: 0.08 },
        ]}
      ...
      <div className="relative z-10">
        ...
        <Section>
          ...
          >
            <span className="section-number block mb-4">002/</span>
          ...
          >
            {sections.map((section) => (
              <Link
                key={section.id}
                href={`#${section.id}`}
                className="text-sm px-4 py-2 bg-white/60 border border-[rgba(28,28,28,0.08)] rounded-lg hover:bg-[#E8E4DE] transition-colors"
              >
          ...
          <Section key={section.id} id={section.id} background={index % 2 === 1 ? 'alt' : 'default'}>
            ...
            >
              <div className="grid lg:grid-cols-2 gap-12">
                <div>
                  <span className="text-xs font-medium text-[#4A4A4A] uppercase tracking-wider mb-4 block">
                    Step {section.number}
                  </span>
                  <h2 className="font-serif text-3xl md:text-4xl font-light mb-6">
                    {section.title}
                  </h2>
                  <div className="prose prose-lg text-[#4A4A4A] whitespace-pre-line">
                      ...
                      if (line.startsWith('**') && line.includes('**')) {
                        ...
                        return (
                          <p key={i} className="mb-3">
                            {parts.map((part, j) => 
                              j % 2 === 1 ? <strong key={j} className="text-[#1C1C1C]">{part}</strong> : part
                            )}
                          </p>
                        );
                      }
                ...
                <div className="lg:sticky lg:top-32 h-fit">
                  <Card className="!p-0 overflow-hidden">
                    <div className="bg-[#1C1C1C] text-[#F6F3EE] p-4 text-xs font-mono">
        ...
        <Section id="privacy" background="alt">
          <motion.div
            ...
            transition={{ duration: 0.6 }}
          >
            <span className="section-number block mb-4">Privacy</span>
            ...
            <div className="grid md:grid-cols-2 gap-8 max-w-4xl">
              {[
                ...
                <motion.div
                  key={index}
                  className="border-l-2 border-[rgba(28,28,28,0.1)] pl-6"
                  initial={{ opacity: 0, x: -20 }}
                  whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.4, delay: index * 0.05 }}
                >
                  <h3 className="font-medium text-[#1C1C1C] mb-2">{item.title}</h3>
        ...
        <Section id="faq">
          <motion.div
            ...
            transition={{ duration: 0.6 }}
          >
            <span className="section-number block mb-4">FAQ</span>
            <h2 className="font-serif text-3xl md:text-4xl font-light mb-8">
              Common questions
            </h2>
            ...
            <div className="space-y-6 max-w-3xl">
              {[
                ...
                <motion.div
                  key={index}
                  className="bg-white/60 border border-[rgba(28,28,28,0.08)] rounded-2xl p-6"
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.4, delay: index * 0.05 }}
                >
                  <h3 className="font-medium text-[#1C1C1C] mb-2">{faq.q}</h3>
